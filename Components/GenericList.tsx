...
export function getWebsiteCell(item: ResultItem) {
    ...
    return <a href={website} target="_blank" rel="noopener noreferrer">{getHostname(website)}</a>
}

export function getEditCell(item: ResultItem) {
    return (
        <Fragment>
            <a href={`https://www.openstreetmap.org/${item.type}/${item.osm_id}`} target="_blank" rel="noreferrer">🗺</a>
            <a href={`https://www.openstreetmap.org/edit?${item.type}=${item.osm_id}`} target="_blank" rel="noreferrer">✍️</a>
        </Fragment>
    )
}
...
export function getOpeningCell(item: ResultItem) {
    ...
    try {
        const parsed = new opening_hours(item.tags['opening_hours'], { lat: parseInt(item.lat, 10), lon: parseInt(item.lon, 10), address: { country_code: 'be', state: "" } })
